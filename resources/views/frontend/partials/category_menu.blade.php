        ...
        @foreach (get_level_zero_categories()->take(10) as $key => $category)
            ...
            <li class="category-nav-element border border-top-0" data-id="{{ $category->id }}">
                <a href="{{ route('products.category', $category->slug) }}"
                    ...
                    <img class="cat-image lazyload mr-2 opacity-100" src="{{ static_asset('assets/img/placeholder.jpg') }}"
                        data-src="{{ uploaded_asset($category->icon) }}" width="50" alt="{{ $category_name }}"
