...
class HomeController extends Controller
{
    ...
    public function verifyotp(Request $request)
    {
        ...
        {    
            $userotp = Session::get('otp'); 
            if($request->otp == $userotp){
            $userregister = Session::get('userRegister');
            $loginuser = Session::get('loginuser');
            if($loginuser)
                {
                    $mobile = Session::get('loginuser');
                    $user = User::where('phone',$mobile)->first();
                    Session::put('userSession',$user->phone);
                    Auth::login($user);
                    $phone = $mobile;
                    ...
                    $user->save();
                    Auth::login($user);
                    $request->session()->forget('userRegister');
                    $request->session()->forget('otp');
                    
                }
                return redirect('/');
            }else{
                return redirect('users/user-otp')->with('flash_message_error','Please enter correct OTP.');
            }
            // return $this->registered($request, $user)
            // ?: redirect($this->redirectPath());
        }
    }
    public function verifyreotp(Request $request)
    {
        if($request->isMethod('post')){
            ...
            $user->otp = $otp;
            $user->save();
            return redirect('users/user-otp')->with('flash_message_error','OTP Resend Successfully.');
        }
    }
    public function userlogin(Request $request)
    {
        if($request->isMethod('post')){
            ...
            if($user){
                Session::put('loginuser',$mobile);
                $otp = mt_rand(100000, 999999);
                $mobile = '91'.$data['phone'];
                $this->usersendOTP($mobile, $otp);
                Session::put('otp', $otp);
                return redirect('/users/user-otp');
            }else{
                return redirect()->back()->with('flash_message_error','User not registered.');
            }
        }
        return view('frontend/user_login');
    }
